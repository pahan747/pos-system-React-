    ...
    <div className="delivery-section">
      ...
      {cartData && (
        <div className="totals">
          ...
          <p>Delivery Fee: <span>${(cartData.deliveryFee || 0).toFixed(2)}</span></p>
          {(cartData.discount || 0) > 0 && (
            <p>Discount: <span>-${(cartData.discount || 0).toFixed(2)}</span></p>
          )}
          <h3>Total: <span>${((cartData.subTotal || 0) + (cartData.deliveryFee || 0)).toFixed(2)}</span></h3>
        </div>
      )}
    </div>
  );
